...
class ManagerController extends Controller
{
    ...
    public function updateLevel(Request $request)
    {
        ...
        $array = [];
        $arr = [];
        foreach($user->group as $key2 => $value0) {
            array_push($arr,$value0->id);
        };

        foreach($user->group as $key => $value) {
            $number = 1;
            if($request->group != null){
                (in_array($value->id,$request->group)) ? $number = 0 : $number = 1;
            }
            else {
                $request->group = [];
            }
            if($number = 1 ) {
                $value->pivot->deleted_at = Carbon::now();
                $value->pivot->save();
            }
            if(in_array($value->id,$request->group)) {
